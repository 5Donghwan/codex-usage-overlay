...
> **서명과 권한 유지:** `build.sh`는 첫 실행 시 로컬 자체 서명 인증서
> (`CodexUsageOverlay Local Signing`)를 로그인 키체인에 자동 생성하고, 이후 빌드는 항상
> 이 인증서로 서명합니다. macOS는 권한을 "번들 ID + 인증서"에 묶으므로 소스를 고쳐
> 재빌드해도 접근성 권한이 유지됩니다. 승인은 맥 한 대당 최초 한 번만 필요합니다
> (다른 맥에서는 `./build.sh`가 그 맥용 인증서를 새로 만들고, 거기서 한 번 더 승인).
> 배치·크기·색상 임계값은 `tunables.json`에서 조정하면 재빌드 없이 약 2초 안에 반영됩니다.
